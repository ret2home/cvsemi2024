...
def calculate_intersection(x1, y1, x2, y2, theta):
    ...
    if theta==90:
        ...
        x4,y4=1000*math.cos(theta/180*math.pi),1000*math.sin(theta/180*math.pi)
    # 線分1の係数
    A1 = y2 - y1
    B1 = x1 - x2
    C1 = x2 * y1 - x1 * y2
    ...
    # 交点が両方の線分上にあるか確認
    if (min(x1, x2)-eps <= x <= max(x1, x2)+eps and min(y1, y2)-eps <= y <= max(y1, y2)+eps and
        min(x3, x4)-eps <= x <= max(x3, x4)+eps and min(y3, y4)-eps <= y <= max(y3, y4)+eps):
        return (x, y)
    else:
        return None
